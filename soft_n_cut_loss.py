...
def edge_weights(flatten_image, rows, cols, std_intensity=3, std_position=1, radius=5):
    ...
    xx, yy = torch.meshgrid(torch.range(rows), torch.range(cols))
    ...
    #dist_weight = tf.cast(dist_weight, tf.float32) # Is it already float32?
...
def numerator(k_class_prob,weights):
    ...
    '''
    #k_class_prob = torch.view(k_class_prob, (-1,))
...
def denominator(k_class_prob,weights):
    ...
    weights : edge weights	n*n tensor
    '''
    #k_class_prob = tf.cast(k_class_prob, tf.float32) # should already be float32
    k_class_prob = k_class_prob.view(-1)
    return torch.sum(
        torch.mul(
            weights,
